...
| 项目介绍   
  ...
  - **Cityscape数据集**  
    Cityscapes是关于城市街道场景的语义理解图片数据集。它主要包含来自50个不同城市的街道场景，拥有5000张在城市环境中驾驶场景的高质量像素级注释图像。其中训练图像2975张，验证图像500张,测试图像152张， 共计35个类别；实际研究和使用时可选择部分类别。往往此外，它还有20000张粗糙标注的图像(gt coarse)。本项目使用了原始图片leftImg8bit_trainvaltest.zip 和 标记文件gtFine_trainvaltest.zip，划定了道路安全上最重要常见的15类，在验证集val(500张)上测试模型分割效果。
  - **Intel OneAPI**  
